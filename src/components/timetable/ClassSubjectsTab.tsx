...
function SubjectRow({ subject, entry, onChange, langLevels, schedSlots }: {
  ...
  const totalPeriods = entry.periodsPerWeek + (entry.hasLabSession ? entry.labPeriodsPerWeek : 0);

  return (
    <div className={`border rounded-xl overflow-hidden transition-colors ${entry.included ? `${colorClass} border-opacity-60` : 'bg-white border-gray-100'}`}>
      {/* ── Main row ── */}
      <div className="flex items-center gap-3 px-4 py-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className={`text-sm font-semibold font-dm-sans ${entry.included ? '' : 'text-gray-400'}`}>
              {subject.name}
            </span>
            ...
            {entry.included && !entry.isCompulsory && (
        ...
        {/* ── Period controls — always visible when included ── */}
        {entry.included && (
          <div className="flex items-center gap-1.5 flex-shrink-0">

            {/* Theory periods pill */}
            <div className="flex items-center gap-1 bg-white/70 border border-black/10 rounded-lg px-2 py-1.5">
              <span className="text-[10px] text-gray-500 font-medium leading-none">Theory</span>
              <input
                type="number" value={entry.periodsPerWeek} min={1} max={14}
                onChange={e => onChange({ periodsPerWeek: Math.max(1, Math.min(14, Number(e.target.value))) })}
                className="w-8 text-sm font-bold text-center bg-transparent border-none outline-none text-navy leading-none"
              />
              <span className="text-[10px] text-gray-400 leading-none">p/w</span>
            </div>

            {/* Lab pill — clickable to toggle, editable inline when active */}
            {canHaveLab && (
              <div
                onClick={() => onChange({ hasLabSession: !entry.hasLabSession })}
                className={`flex items-center gap-1 rounded-lg px-2 py-1.5 border cursor-pointer select-none transition-all ${
                  entry.hasLabSession
                    ? 'bg-teal-50 border-teal-300'
                    : 'bg-white/40 border-dashed border-gray-200 hover:border-teal-200 hover:bg-teal-50/40'
                }`}>
                <FlaskConical className={`w-3 h-3 flex-shrink-0 ${entry.hasLabSession ? 'text-teal-600' : 'text-gray-300'}`} />
                {entry.hasLabSession ? (
                  <>
                    <span className="text-[10px] text-teal-600 font-medium leading-none">Lab</span>
                    <input
                      type="number" value={entry.labPeriodsPerWeek} min={2} max={8} step={2}
                      onClick={e => e.stopPropagation()}
                      onChange={e => onChange({ labPeriodsPerWeek: Math.max(2, Math.min(8, Number(e.target.value))) })}
                      className="w-7 text-sm font-bold text-center bg-transparent border-none outline-none text-teal-700 leading-none"
                    />
                    <span className="text-[10px] text-teal-500 leading-none">p/w</span>
                  </>
                ) : (
                  <span className="text-[10px] text-gray-400 leading-none">+ Lab</span>
                )}
              </div>
            )}

            {/* Total badge when lab is active */}
            {canHaveLab && entry.hasLabSession && (
              <span className="text-[11px] font-bold text-gray-500 whitespace-nowrap">= {totalPeriods} p/w</span>
            )}
      ...
      {entry.included && expanded && (
        <div className="px-4 pb-4 pt-2 border-t border-black/5 grid grid-cols-2 gap-3 bg-white/50">
          {isLang && (
            <div>
          ...
          <div>
            <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide block mb-1">Scheduling Slot</label>
          ...
          <div className="col-span-2">
            <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide block mb-1">Max periods per day</label>
            <div className="flex items-center gap-2">
              {[1, 2, 3, 0].map(n => (
                <button key={n} onClick={() => onChange({ maxPerDay: n })}
          ...
          {canHaveLab && entry.hasLabSession && (
            <p className="col-span-2 text-[10px] text-teal-600 bg-teal-50 rounded-lg px-3 py-1.5">
              Generator will schedule {entry.labPeriodsPerWeek / 2} back-to-back lab double-period{entry.labPeriodsPerWeek > 2 ? 's' : ''} per week
            </p>
          )}
        </div>
      )}
