...
import MHSTimetableGrid     from './MHSTimetableGrid';
import MHSAIGenerator       from './MHSAIGenerator';
import MHSSubstitutionPanel from './MHSSubstitutionPanel';
...
export default function MHSTimetablePage() {
  ...
  return (
    <div className="p-6 space-y-5 animate-fadeIn">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            WBBSE · Academic Year 2026-27 · Generated {new Date(meta.generatedAt).toLocaleString('en-IN', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
          </p>
        </div>
        ...
        {tabs.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-dm-sans transition-all duration-150 ${isActive ? 'bg-white text-navy font-semibold shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
              <Icon className="w-4 h-4" />
              {t.label}
              {t.badge && (
                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${t.badge === 'AI' ? 'bg-teal text-white' : 'bg-coral text-white'}`}>{t.badge}</span>
              )}
            </button>
          );
        })}
      ...
      <div>
        {activeTab === 'view'         && <MHSTimetableGrid />}
        {activeTab === 'generate'     && <MHSAIGenerator />}
        {activeTab === 'substitution' && <MHSSubstitutionPanel />}
      </div>
    </div>
  );
}
