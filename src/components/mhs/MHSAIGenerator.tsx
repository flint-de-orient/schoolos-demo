...
import { Brain, CheckCircle2, Loader2, Zap, RefreshCw, Shield, Gauge, BookOpenCheck, ChevronDown, ChevronUp } from 'lucide-react';
...
const STEPS = [
  ...
  { icon: '🔍', label: 'Mapping WBBSE subjects to sections & periods…' },
...
const CONSTRAINTS = [
  ...
  'Core WBBSE subjects distributed across Mon–Fri',
  'Saturday capped at 5 periods per class',
];

const { qualityScore } = timetableData.meta;

export default function MHSAIGenerator() {
  ...
  async function generate() {
    ...
    setDone(false); setGenerating(true); setStepIndex(0);
    for (let i = 1; i <= STEPS.length; i++) { await new Promise(r => setTimeout(r, 900)); setStepIndex(i); }
    setGenerating(false); setDone(true);
    toast.success('Timetable generated — 0 conflicts');
  }
      ...
      <div className="col-span-2 space-y-5">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {classesData.map(c => (
                <label key={c.id} className="flex items-center gap-1.5 cursor-pointer">
                  <input type="checkbox" checked={selected.has(c.id)} onChange={() => toggleClass(c.id)} className="w-3.5 h-3.5 accent-navy" />
                  <span className="text-xs text-gray-700">{c.id}</span>
                </label>
              ))}
            </div>
          </div>
          <div className="mb-4">
            <label className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 block">Periods / Day</label>
            <div className="flex gap-2">
              {[6,7,8].map(n => (
                <button key={n} onClick={() => setPeriodsPerDay(n)} className={`px-4 py-1.5 rounded-lg text-sm font-sora font-semibold transition-colors ${periodsPerDay === n ? 'bg-navy text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{n}</button>
              ))}
            </div>
          </div>
          <button onClick={() => setShowConstraints(!showConstraints)} className="w-full flex items-center justify-between text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
          ...
          {showConstraints && (
            <ul className="space-y-1.5">
              {CONSTRAINTS.map((c, i) => (
                <li key={i} className="flex gap-2 text-xs text-gray-600"><Shield className="w-3.5 h-3.5 text-teal flex-shrink-0 mt-0.5" />{c}</li>
              ))}
            </ul>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${onLeave > 0 ? 'bg-amber/10 text-amber' : 'bg-green/10 text-green'}`}>{onLeave} on leave</span>
          </div>
          ...
          <div className="mt-2 pt-2 border-t border-gray-100 text-xs text-gray-400">{availableTeachers} of {teachersData.length} available</div>
        </div>

        <button onClick={generate} disabled={generating} className="w-full flex items-center justify-center gap-2 bg-gold hover:bg-gold/90 text-navy font-sora font-bold py-3 rounded-xl transition-colors disabled:opacity-60">
      ...
      <div className="col-span-3">
        {!generating && !done && (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 flex flex-col items-center justify-center text-center min-h-[400px]">
            <div className="w-16 h-16 bg-navy/5 rounded-2xl flex items-center justify-center mb-4"><Brain className="w-8 h-8 text-navy/40" /></div>
            <h3 className="font-sora font-semibold text-gray-700 mb-2">AI Timetable Engine Ready</h3>
            <p className="text-sm text-gray-400 max-w-xs">Configure options and click &quot;Generate&quot; to create a conflict-free WBBSE timetable.</p>
          </div>
        )}
        ...
        {generating && (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 min-h-[400px]">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-8 h-8 bg-teal/10 rounded-lg flex items-center justify-center"><Loader2 className="w-4 h-4 text-teal animate-spin" /></div>
              <div>
                <div className="font-sora font-semibold text-gray-900 text-sm">Generating…</div>
            ...
            <div className="mt-4 w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
              <div className="h-1.5 bg-gradient-to-r from-teal to-green rounded-full transition-all duration-700" style={{ width: `${(stepIndex / STEPS.length) * 100}%` }} />
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-xl border border-green/30 shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2"><CheckCircle2 className="w-5 h-5 text-green" /><h3 className="font-sora font-semibold text-gray-900">Generation Complete</h3></div>
              ...
              <div className="p-3 bg-green/5 border border-green/20 rounded-lg text-xs text-green font-semibold">
                ✓ {selected.size} sections · {timetableData.meta.totalSlotsFilled} slots · {timetableData.meta.conflictsFound} conflicts
              </div>
            </div>
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              <div className="flex items-center gap-2 mb-3"><Zap className="w-4 h-4 text-teal" /><span className="font-sora font-semibold text-sm text-gray-900">What the AI optimised</span></div>
              <ul className="space-y-2 text-sm text-gray-600">
                <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-green flex-shrink-0 mt-0.5" />T006 (on leave) — English periods redistributed to T005 & T007</li>
                <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-green flex-shrink-0 mt-0.5" />WBBSE Maths spread across all 6 days for IX–X to reduce cramming</li>
                <li className="flex gap-2"><CheckCircle2 className="w-4 h-4 text-green flex-shrink-0 mt-0.5" />Physical Education placed in early periods to avoid afternoon heat</li>
              </ul>
            </div>
            <div className="flex gap-3">
              <button onClick={generate} className="flex-1 flex items-center justify-center gap-2 border border-gray-200 text-gray-700 py-2.5 rounded-xl hover:bg-gray-50 text-sm transition-colors">
